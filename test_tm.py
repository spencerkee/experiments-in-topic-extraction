...
def create_document_list(movie_id_to_rating,user_rating_history):
    doc_set = []
    movies = sorted(movie_id_to_rating.keys())[:1000] #not movie_id_to_movie because some movies have no ratings?
    for movie in movies:#can be range(num_movies), for every movie
        doc_tags_list = []
        for user_rating in movie_id_to_rating[movie]:
            if float(user_rating[1]) >= 5:
                for rated_movie in user_rating_history[user_rating[0]]:
                    if rated_movie[0] == movie or (float(rated_movie[1]) >= 5): #don't want to have cyclical associations
                        continue
...
def main(load=['texts','dictionary','corpus','ldamodel'],save=False,topic_num=20,passes_num=20):
    start_time = time.time()
    # designation = str(topic_num) + "_" + str(passes_num) + '_' + 'test'
    designation = 'current_test'
    ...
    # print ('number of movies', len(movie_id_to_movie.keys()))

    texts = [i for i in create_document_list(movie_id_to_rating,user_rating_history) if len(i) >= 100]
    print len(texts)
    # # print ('average document length', sum([len(i) for i in texts])/len(texts)) #1749
    print ('median', statistics.median([len(i) for i in texts]))

    # with open("/home/archie/delete_tm",'w') as f:
    #     for i in texts:
    #         f.write(str(i))
    #         f.write('\n')

    # for movie_id in movie_id_to_rating.keys():
    #     if len(movie_id_to_rating[movie_id]) >= 100:
    #         print movie_id_to_movie[movie_id], len(movie_id_to_rating[movie_id])
...
if __name__ == '__main__':
    ...
    #20000 movies = 2233 seconds
    #2000 movies = 4600 seconds
    #10 = 30 seconds